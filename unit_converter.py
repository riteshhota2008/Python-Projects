...
def do_convert(conversion):
    ...
    print("%s %s's equals %f %s's" % (amount, source_unit, amount * CONVERT_FROM[conversion][source_unit] * CONVERT_TO[conversion][to_unit], to_unit))
...
def currency():
    ...
    request = urlopen(
        'http://rate-exchange.appspot.com/currency?from=' + from_currency + '&to=' + to_currency + '&q=' + amount)
